...
import { CATEGORIES, type CategoryKey } from "@/data/posts";
import { getAllPostMetas, countAllByCategory } from "@/lib/allPosts";
...
const Blog = () => {
  ...
  const featuredPosts = getAllPostMetas().slice(0, 6);
        ...
        <section className="section-padding bg-gradient-to-br from-primary/10 to-secondary/10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
              Những câu chuyện, kiến thức và kinh nghiệm thực tế để giúp bạn phát
              triển và thành công
            </p>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
              {categoryEntries.map(([key, cat]) => {
                const cnt = countAllByCategory(key);
                return (
                  <Link
                    key={key}
                    to={cat.url}
